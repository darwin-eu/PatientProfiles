...
The cohort tables in this example are created with `CohortConstructor`. PatientProfiles then adds variables and summarises the resulting valid cohort tables; cohort creation and modification are outside the scope of PatientProfiles and can be handled by other dedicated OMOP cohort-generation packages.
