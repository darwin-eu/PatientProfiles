...
In this vignette, `CohortConstructor` is used to create the ankle sprain cohort. PatientProfiles then works with the resulting valid cohort table; cohort creation and modification are outside the scope of PatientProfiles and can similarly be handled by other dedicated OMOP cohort-generation packages.

## Adding variables from concept sets

For this vignette we'll use the GiBleed synthetic dataset. First let's create our cohort of interest, individuals with an ankle sprain.
